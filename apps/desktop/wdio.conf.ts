...
/**
 * The profile one spec file runs on, made for it and thrown away after the run (design D3-06).
 *
 * A suite that opened the profile of this machine would migrate it, back it up and write to it
 * — which is the one thing no test is allowed to do. The application accepts the folder because
 * it is a `dev` build; a package would ignore the argument.
 *
 * One folder per spec file, because a profile is state and state shared between spec files is
...
export const config: WebdriverIO.Config = {
  ...
  specs: ['./e2e/**/*.e2e.ts'],
  services: ['electron'],
  // One capability per spec file, each pointed at a profile of its own.
  //
  // `--user-data-dir` as well as `--profile-dir`, and both the same folder: the driver looks for
  // the port file Chromium writes inside the user data directory it chose, and the application
  // moves `userData` onto its profile. Told the same folder twice, the two agree; told nothing,
  // they disagree and no session is ever created.
  capabilities: SPECS.map((spec) => ({
    browserName: 'electron',
    specs: [`./e2e/${spec}`],
    'wdio:electronServiceOptions': {
      appEntryPoint: `${application}/dist/main/index.js`,
      appArgs: [`--user-data-dir=${e2eProfileOf(spec)}`, `--profile-dir=${e2eProfileOf(spec)}`],
    },
  })),
  logLevel: 'warn',
  reporters: ['spec'],
