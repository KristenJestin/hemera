/**
 * One instance per profile, and two profiles side by side (design D3-06).
 *
 * The suite already drives one running application. What it checks here is what a *second*
 * start does: on the same profile it has to step aside and hand the window back, and on another
 * profile it has to run, because that is the whole point of having a `dev` profile at all.
 *
...
import { DATABASE_FILE } from '../src/profile/index.ts'
import { e2eProfileOf } from '../wdio.conf.ts'

const application = dirname(dirname(fileURLToPath(import.meta.url)))

/** The profile this very spec file runs on, named after it exactly as the run named it. */
const E2E_PROFILE = e2eProfileOf('profile.e2e.ts')
const binary: string = createRequire(join(application, 'package.json'))('electron')

/** Another application, started on a profile of its own, and the promise of its ending. */
interface Started {
  process: ChildProcess
  ended: Promise<number | null>
}

/** Starts another application on a profile of its own choosing, and answers when it ends. */
function start(profile: string): Started {
  const { ELECTRON_RUN_AS_NODE: _runAsNode, ...environment } = process.env
  const started = spawn(
    binary,
    [join(application, 'dist', 'main', 'index.js'), `--profile-dir=${profile}`],
...
describe('La seconde instance s’efface', () => {
  it('a second start on the same profile quits without opening a window of its own', async () => {
    const before = await browser.electron.execute(
      (electron) => electron.BrowserWindow.getAllWindows().length,
    )

    const second = start(E2E_PROFILE)
  ...
  it('the profile says in its own log that an instance stepped aside', () => {
    const log = join(E2E_PROFILE, DIAGNOSTIC_FILE)
    expect(existsSync(log)).toBe(true)
  })
})

describe('Dev et prod tournent côte à côte', () => {
  it('a start on another profile runs alongside the first', async () => {
...
describe('La base vit dans le process dédié', () => {
  it('the profile holds a database, and a process of its own is what opened it', async () => {
    ...
    const held = opened.utilities.find((one) => one.name?.includes('profile') === true)
    expect(held).toBeDefined()
    expect(held?.pid).not.toBe(opened.main)

    expect(existsSync(join(E2E_PROFILE, DATABASE_FILE))).toBe(true)
  })

  it('the log says the process that holds it opened it, and says so in its own name', () => {
    const written = readFileSync(join(E2E_PROFILE, DIAGNOSTIC_FILE), 'utf8')
    expect(written).toContain('[profile] opened the database')
  })
})

describe('La sidebar survit au redémarrage', () => {
  it('folding the sidebar by hand is what the profile is left holding', async () => {
    // Through the control the user has, not through the channel: what is under test is the
    // round trip from a hand to the profile, and a channel called behind the page's back is a
    ...
    const hint = JSON.parse(readFileSync(join(E2E_PROFILE, 'display.json'), 'utf8')) as {
...
describe('Le thème survit au redémarrage', () => {
  it('a theme written to the profile is the theme the profile answers afterwards', async () => {
    await browser.execute(
