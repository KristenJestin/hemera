...
15 September 2026. This repository is the product monorepo and holds code only. Product rules,
OpenSpec changes, verification reports and spike reports live in the parent documentation
folder (`../docs`, `../openspec`): agents are launched from there, never from here, and no
`openspec/` folder exists in this repository. Read the current lot's `proposal.md`, `specs/`,
`design.md` and `tasks.md` before touching code.
...
`test` blocks. `reports/` is generated by `pnpm report` and `pnpm test` and is not committed:
the reports that prove a task are filed in the documentation folder, beside the lot.
