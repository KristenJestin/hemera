...
- **Main process**: ESM, minimal. It resolves the channel and the profile before anything else,
  moves `userData` onto the profile, takes the single-instance lock, creates the window,
  registers the IPC channels, starts the process that holds the database and produces the
  environment report. Everything that must precede `ready` is awaited at the top
...
- **Profile**: the named `utilityProcess` that holds the database, ESM on Node, no window and no
...
- Profile lives per OS (`LOCALAPPDATA` / `XDG_DATA_HOME`), never in `/tmp`, never inside
  a product Workspace. `prod` and `beta` share one profile — the beta is the application the
  user runs every day, on real data — and `dev` has its own. The channel is engraved in a
  package's manifest when it is built and cannot be changed at run time.
- A `dev` build, and only a `dev` build, accepts `--profile-dir <path>` for a throwaway profile,
  and refuses a path that resolves to the `prod` one. That is how the end-to-end suite and every
  trial run without ever opening the real profile.
...
alone: `apps/desktop/src/profile/` is the only folder allowed to import `node:sqlite`, Drizzle
or the Effect SQLite client, and `tools/boundaries.ts` refuses it anywhere else. The main
process asks that process by name — `preferences.read`, `preferences.write`, `profile.status`,
declared in `packages/ipc/src/profile.ts` — and never opens the file itself.
